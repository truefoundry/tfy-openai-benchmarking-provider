...
def streaming_data_generator(content, model="gpt-3.5-turbo"):
...
async def completion(request: Request, model: str = None):
    ...
    requested_model = body.get("model") or query.get("model") or model or "gpt-3.5-turbo"
    stream = as_bool(body.get("stream")) or as_bool(query.get("stream"))
    ...
    if stream:
        return StreamingResponse(
            content=streaming_data_generator(content, requested_model),
